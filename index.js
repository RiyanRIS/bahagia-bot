...
async function main() {
  ...
  conn.on('chat-update', async (mek) => {
    try {
      ...
      // TEBAK LIRIK HANDLER
      const tebaktebakanhandler = async () => {
        let asahotakk1 = JSON.parse(fs.readFileSync("./src/data/tebaktebakan.json"))

        let isAsahOtak = false
        let resAsahOtaak1 = []
        let indexkeberapa = null

        // cek apakah pengirim sedang mengerjakan tebak lirik
        asahotakk1.forEach((i, el) => {
          if (from == i.from) {
            isAsahOtak = true
            isOnGame = true
        ...
        if (isAsahOtak && kalimat.toLowerCase() != "skip") { // kondisi jika pengirim sedang mengerjakan tebak lirik
          ...
          if (jawab == resAsahOtaak1[0].jawaban.toLowerCase()) { // kondisi jika jawaban benar
            ...
            let dataasahhapus = JSON.parse(fs.readFileSync("./src/data/tebaktebakan.json"))
            let idxasah
            dataasahhapus.forEach((i, el) => {
              if (from == i.from) {
                idxasah = el
            ...
            dataasahhapus.splice(idxasah, 1)
            fs.writeFileSync("./src/data/tebaktebakan.json", JSON.stringify(dataasahhapus))
          } else { // kondisi jika jawaban salah
            if (resAsahOtaak1[0].percobaan == 1) {
              reply("salah")
            } else if (resAsahOtaak1[0].percobaan == 2) {
            ...
            let dataasahhapus1 = JSON.parse(fs.readFileSync("./src/data/tebaktebakan.json"))
            let idxasah1
            // Hapus dulu data lama
            dataasahhapus1.forEach((i, el) => {
              if (from == i.from) {
            ...
            dataasahhapus1.push(data)
            fs.writeFileSync("./src/data/tebaktebakan.json", JSON.stringify(dataasahhapus1))
          }
        }
        ...
        if (isAsahOtak && kalimat.toLowerCase() == "skip") {
          ...
          if (indexkeberapa != null) {
            asahotakk1.splice(indexkeberapa, 1)
            fs.writeFileSync("./src/data/tebaktebakan.json", JSON.stringify(asahotakk1))
          }
          sendButMessage(from, "Mau main lagi gak?.\n", `Bot Bahagia`, [{
            buttonId: `${prefix}tebaktebakan`,
            buttonText: {
      ...
      // TEBAK LIRIK HANDLER
      const tebaklirikhandler = async () => {
        let asahotakk1 = JSON.parse(fs.readFileSync("./src/data/tebaklirik.json"))
        ...
        if (isAsahOtak && kalimat.toLowerCase() != "skip") { // kondisi jika pengirim sedang mengerjakan tebak lirik
          ...
          if (jawab == resAsahOtaak1[0].jawaban.toLowerCase()) { // kondisi jika jawaban benar
            ...
            let dataasahhapus = JSON.parse(fs.readFileSync("./src/data/tebaklirik.json"))
            let idxasah
            dataasahhapus.forEach((i, el) => {
              if (from == i.from) {
                idxasah = el
            ...
            dataasahhapus.splice(idxasah, 1)
            fs.writeFileSync("./src/data/tebaklirik.json", JSON.stringify(dataasahhapus))
          } else { // kondisi jika jawaban salah
            if (resAsahOtaak1[0].percobaan == 1) {
              reply("salah")
            } else if (resAsahOtaak1[0].percobaan == 2) {
            ...
            let dataasahhapus1 = JSON.parse(fs.readFileSync("./src/data/tebaklirik.json"))
            let idxasah1
            // Hapus dulu data lama
            dataasahhapus1.forEach((i, el) => {
              if (from == i.from) {
            ...
            dataasahhapus1.push(data)
            fs.writeFileSync("./src/data/tebaklirik.json", JSON.stringify(dataasahhapus1))
          }
        }
        ...
        if (isAsahOtak && kalimat.toLowerCase() == "skip") {
          ...
          if (indexkeberapa != null) {
            asahotakk1.splice(indexkeberapa, 1)
            fs.writeFileSync("./src/data/tebaklirik.json", JSON.stringify(asahotakk1))
          }
          sendButMessage(from, "Mau main lagi gak?.\n", `Bot Bahagia`, [{
            buttonId: `${prefix}tebaklirik`,
            buttonText: {
      ...
      switch (command) {
        ...
        case 'asahotak':
          await pengumum()
            .then(async (res) => {
              await conn.sendMessage(from, res.soal.trim(), text).then((resp) => {
                  // buat perintah bahwa si x sedang mengerjakan games asahotak
                  data = {
                    from: from,
                    percobaan: 1,
                    soal: res.soal.trim(),
                    jawaban: res.jawaban.trim(),
                    status: 0
                  }
                  let tebakpribahasabaru = JSON.parse(fs.readFileSync("./src/data/asahotak.json"))
                  tebakpribahasabaru.push(data)
                  fs.writeFileSync("./src/data/asahotak.json", JSON.stringify(tebakpribahasabaru))

                  // mulai hitung mundur 2 menit dari sekarang, kalo belum terjawab, munculin jawabanya
                  let tedd = 1
                  const intervRemind = setInterval(async () => {
                    let datatebakpribahasa = JSON.parse(fs.readFileSync("./src/data/asahotak.json"))
                    let isTebakPribahasa = false
                    ...
                    if (tedd++ == 120) { // kondisi jika sudah 2 menit belum terjawab
                      ...
                      datatsdebakpribahasa.splice(inddfx, 1)
                      fs.writeFileSync("./src/data/asahotak.json", JSON.stringify(datatsdebakpribahasa))
                      clearInterval(intervRemind)
                    }
                  }, 1000);
                })
        ...
        case 'tebaklirik':
          await tebaklirik()
            .then(async (res) => {
              let sooal = `Soal:\n\n*${res.soal.trim()}*\n\n_Kami tunggu 2 menit mulai dari sekarang_`
              await conn.sendMessage(from, sooal, text).then((resp) => {
                  // buat perintah bahwa si x sedang mengerjakan games asahotak
                  data = {
                    from: from,
                    percobaan: 1,
                  ...
                  let tebakpribahasabaru = JSON.parse(fs.readFileSync("./src/data/tebaklirik.json"))
                  tebakpribahasabaru.push(data)
                  fs.writeFileSync("./src/data/tebaklirik.json", JSON.stringify(tebakpribahasabaru))

                  // mulai hitung mundur 2 menit dari sekarang, kalo belum terjawab, munculin jawabanya
                  let tedd = 1
                  const intervRemind = setInterval(async () => {
                    let datatebakpribahasa = JSON.parse(fs.readFileSync("./src/data/tebaklirik.json"))
                    let isTebakPribahasa = false
                    ...
                    if (tedd++ == 120) { // kondisi jika sudah 2 menit belum terjawab
                      ...
                      let datatsdebakpribahasa = JSON.parse(fs.readFileSync("./src/data/tebaklirik.json"))
                      let inddfx
                      datatebakpribahasa.forEach((i, el) => {
                        if (from == i.from) {
                          inddfx = el
                      ...
                      datatsdebakpribahasa.splice(inddfx, 1)
                      fs.writeFileSync("./src/data/tebaklirik.json", JSON.stringify(datatsdebakpribahasa))
                      clearInterval(intervRemind)
                    }
                  }, 1000);
                })
          ...
          await tebaktebakan()
            .then(async (res) => {
              ...
              await conn.sendMessage(from, sooal, text).then((resp) => {
                  ...
                  let tebakpribahasabaru = JSON.parse(fs.readFileSync("./src/data/tebaktebakan.json"))
                  tebakpribahasabaru.push(data)
                  fs.writeFileSync("./src/data/tebaktebakan.json", JSON.stringify(tebakpribahasabaru))

                  // mulai hitung mundur 2 menit dari sekarang, kalo belum terjawab, munculin jawabanya
                  let tedd = 1
                  const intervRemind = setInterval(async () => {
                    let datatebakpribahasa = JSON.parse(fs.readFileSync("./src/data/tebaktebakan.json"))
                    let isTebakPribahasa = false
                    ...
                    if (tedd++ == 120) { // kondisi jika sudah 2 menit belum terjawab
                      ...
                      let datatsdebakpribahasa = JSON.parse(fs.readFileSync("./src/data/tebaktebakan.json"))
                      let inddfx
                      datatebakpribahasa.forEach((i, el) => {
                        if (from == i.from) {
                          inddfx = el
                      ...
                      datatsdebakpribahasa.splice(inddfx, 1)
                      fs.writeFileSync("./src/data/tebaktebakan.json", JSON.stringify(datatsdebakpribahasa))
                      clearInterval(intervRemind)
                    }
                  }, 1000);
                })
          ...
          await susun()
            .then(async (res) => {
              ...
              await conn.sendMessage(from, soall, text).then((resp) => {
                  ...
                  let tebakpribahasabaru = JSON.parse(fs.readFileSync("./src/data/susunkata.json"))
                  tebakpribahasabaru.push(data)
                  fs.writeFileSync("./src/data/susunkata.json", JSON.stringify(tebakpribahasabaru))

                  // mulai hitung mundur 2 menit dari sekarang, kalo belum terjawab, munculin jawabanya
                  let tedd = 1
                  const intervRemind = setInterval(async () => {
                    let datatebakpribahasa = JSON.parse(fs.readFileSync("./src/data/susunkata.json"))
                    let isTebakPribahasa = false
                    ...
                    if (tedd++ == 120) { // kondisi jika sudah 2 menit belum terjawab
                      ...
                      datatsdebakpribahasa.splice(inddfx, 1)
                      fs.writeFileSync("./src/data/susunkata.json", JSON.stringify(datatsdebakpribahasa))
                      clearInterval(intervRemind)
                    }
                  }, 1000);
                })
