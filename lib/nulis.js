...
module.exports.nulis = (args, jenis = "bukukanan") => {
  return new Promise((resolve, reject) => {
    ...
    } else if(jenis == "entermanual"){
      entermanual(txt, nama, no, kelas).then((res) => {
        resolve(res)
      }).catch((e) => reject(e))
...
const entermanual = async(text, nama = null, no = null, kelas = null) => {
  return new Promise((resolve, reject) => {
    const ran = Math.floor(Math.random() * font.length)
    // const splitText = text.replace(/(\S+\s*){1,11}/g, '$&\n')
...
const bukukanan2 = async(text, nama = null, no = null, kelas = null) => {
