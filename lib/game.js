...
module.exports.tebaklirik = () => {
  return new Promise((resolve, reject) => {
    axios.get("https://raw.githubusercontent.com/tulungagungcode/bot_source/master/tebaklirik/satu.json")
      .then(({data}) => {
        const rndmm = Math.floor(Math.random() * data.length) + 1
        resolve(data[rndmm])
      })
...
module.exports.tebaktebakan = () => {
  return new Promise((resolve, reject) => {
    axios.get("https://raw.githubusercontent.com/tulungagungcode/bot_source/master/tebaktebakan/sati.json")
      .then(({data}) => {
        const rndmm = Math.floor(Math.random() * data.length) + 1
        resolve(data[rndmm])
      })
